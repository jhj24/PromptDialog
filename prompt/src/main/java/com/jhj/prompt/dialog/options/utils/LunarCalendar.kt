package com.jhj.prompt.dialog.options.utils
...
object LunarCalendar {
     ...
     */
    fun lunarToSolar(year: Int, month: Int, monthDay: Int, isLeapMonth: Boolean): IntArray {
        var mYear = year
        var dayOffset: Int
        val leapMonth: Int
        var i = 1

        if (mYear < MIN_YEAR || mYear > MAX_YEAR || month < 1 || month > 12
        ...
        dayOffset = (LUNAR_INFO[mYear - MIN_YEAR] and 0x001F) - 1

        if (LUNAR_INFO[mYear - MIN_YEAR] and 0x0060 shr 5 == 2)
            dayOffset += 31

        while (i < month) {
            dayOffset += if (LUNAR_INFO[mYear - MIN_YEAR] and (0x80000 shr i - 1) == 0) 29 else 30
            i++
        }

        dayOffset += monthDay
        leapMonth = LUNAR_INFO[mYear - MIN_YEAR] and 0xf00000 shr 20

        // 这一年有闰月
        if (leapMonth != 0) {
            if (month > leapMonth || month == leapMonth && isLeapMonth) {
                dayOffset += if (LUNAR_INFO[mYear - MIN_YEAR] and (0x80000 shr month - 1) == 0) 29 else 30
            }
        }

        if (dayOffset > 366 || mYear % 4 != 0 && dayOffset > 365) {
            mYear += 1
            dayOffset -= if (mYear % 4 == 1) 366 else 365
        }
        ...
        while (i < 13) {
            ...
            if (mYear % 4 == 0 && i == 2 && iPos + 1 == dayOffset) {
                solarInfo[1] = i
    ...
    fun solarToLunar(year: Int, month: Int, monthDay: Int): IntArray {
        ...
        var offset = solarToInt(year, month,
    ...
    fun solarToLunarDeprecated(year: Int, month: Int, monthDay: Int): IntArray {
        ...
        if (leapMonth != 0 && iMonth > leapMonth) {
            --iMonth
            Log.i("----------->", year.toString() + "-" + month + "-" + monthDay + "====>" + iMonth + "-" + leapMonth)
            if (iMonth == leapMonth) {
                isLeap = true
            }
        }
    ...
    private fun solarToInt(y: Int, m: Int, d: Int): Long {
        ...
        month = (month + 9) % 12
        year -= month / 10
        return (365 * year + year / 4 - year / 100 + year / 400 + (month * 306 + 5) / 10
                + (d - 1)).toLong()
    }
}
